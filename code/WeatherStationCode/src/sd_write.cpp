...
String sdWriteReadings(Readings data, String filename) {
  ...
  // fails
  String formattedData = data.dateTime + "," + String(data.temperature) + "," +
  ...
  if (cardMounted) {
    ...
    // DATE/TIME, TEMPERATURE, HUMIDITY, PRESSURE, BATTERY VOLTAGE, BATTERY
    // PERCENTAGE
    ...
    if (myFile) {
      ...
      // Serial.println("Error writing to " + filename + ", file failed to
      // open.");
    }

  } else {
    // Serial.println("Error writing to " + filename + ", card failed to
    // mount.");
  }
